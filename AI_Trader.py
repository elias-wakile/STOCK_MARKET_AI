...
def run_trader(trader, data):
    ...
    for t in tqdm(range(data_samples)):
        ...
        if len(trader.memory) > batch_size and t % (batch_size / 4) == 0  :  # todo: I add and t % (batch_size / 2) == 0  mey mastic
...
if __name__ == "__main__":
    ...
    for episode in range(1, episodes + 1):
        ...
        run_trader(trader, train)

        if episode % 5 == 0:
            trader.model.save("ai_trader_{}.h5".format(episode))
    run_trader(trader, test)
    # total_profit = 0
    # state = state_creator(test, 0)
    # data_samples = test.shape[0] - 1
    # for t in tqdm(range(data_samples)):
    #
    #     action = trader.trade(state)
    #
    #     next_state = state_creator(test, t + 1)
    #     reward = 0
    #     if action == 1:  # Buying
    #         trader.inventory.append(test['Close'][t])
    #         print("AI Trader bought: ", stocks_price_format(test['Close'][t]))
    #
    #     elif action == 2 and len(trader.inventory) > 0:  # Selling
    #         buy_price = trader.inventory.pop(0)
    #
    #         reward = test['Close'][t] - buy_price
    #         total_profit += test['Close'][t] - buy_price
    #         print("AI Trader sold: ", stocks_price_format(test['Close'][t]),
    #               " Profit: " + stocks_price_format(test['Close'][t] - buy_price))
    #
    #     if t == data_samples - 1:
    #         done = True
    #     else:
    ...
    #         print("########################")
    #
    #     if len(trader.memory) > batch_size and t % (batch_size) == 0:
    #         trader.batch_train(batch_size)
