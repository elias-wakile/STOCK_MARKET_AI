...
def run_trader(neuralNet, porfolio, batch_size, stock_names):
    i = 0
    done = False
    ...
    for t in range(data_samples):
        ...
        for ind, name in enumerate(stock_names):
            action.append(neuralNet.act([states[ind]]) - int(neuralNet.action_space / 2))  # make this to be between -1 to 1
